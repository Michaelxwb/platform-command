...
export function resolvePath(path, context, fallback) {
  ...
  // `steps.<id>.rows` 是命令步运行时产出的行集合，plan 阶段尚不存在（extract 占位里没有），
  // 视为延迟命名空间，保留占位不报 UNRESOLVED（与 runtime.* 同理）。
  if (/^steps\.[^.]+\.rows$/.test(path)) {
    const stepId = path.split('.')[1];
    const stepObj = context.steps && context.steps[stepId];
    if (!stepObj || !Object.prototype.hasOwnProperty.call(stepObj, 'rows')) return `{{${path}}}`;
  }
