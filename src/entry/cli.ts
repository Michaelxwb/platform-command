...
import { formatHumanReadable, runNaturalLanguage } from '../nl/nl.js';
import { buildAgentManifest, describeCommand, explainNaturalLanguage } from '../model/describe.js';
import { doctorAll, doctorCommand, environmentDoctor } from '../model/doctor.js';
import { initCommandScaffold } from '../model/init.js';
import { listRuns, summarizeRuns } from '../io/runs.js';
import {
  buildScheduleSpec,
...
import { parseKeyValues, printJson } from '../shared/utils.js';
...
Usage:
  ...
  platform-command session import-cookie --host <域名> (--cookie "<整串>" | --cookie-file <文件>) [--out <path>]
  platform-command session login --url <登录页> [--out <path>]
  platform-command session status [--state <path>]
  platform-command daemon start [--max-workers <n>] [--once]
  platform-command daemon status|stop
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg.startsWith('--')) {
      ...
      if (['dryRun', 'dryRunCommand', 'confirm', 'headed', 'executeReal', 'json', 'summary', 'force', 'once'].includes(key)) out[key] = true;
      else out[key] = argv[++i];
    } else {
      out._.push(arg);
    }
...
async function main() {
  ...
  if (cmd === 'session') {
    ...
    const { importCookieState, sessionStatus, loginAndSaveState } = await import('../adapter/session_import.js');
    if (sub === 'status') { printJson(sessionStatus({ state: args.state })); return; }
    if (sub === 'import-cookie') { printJson(importCookieState({ host: args.host, cookie: args.cookie, cookieFile: args.cookieFile, out: args.out })); return; }
    if (sub === 'login') { printJson(await loginAndSaveState({ url: args.url, out: args.out })); return; }
    throw new Error(`unknown session subcommand: ${sub}`);
  }
  if (cmd === 'daemon') {
    const sub = args._[0] || 'status';
    const { startDaemon, stopDaemon, daemonStatus } = await import('../schedule/daemon.js');
    if (sub === 'status') { printJson(daemonStatus()); return; }
    if (sub === 'stop') { printJson(stopDaemon()); return; }
    if (sub === 'start') {
      const opts = { maxWorkers: args.maxWorkers, commandsDir: args.commandsDir };
      if (args.once) { printJson(await startDaemon({ ...opts, once: true })); return; }
      await startDaemon(opts); // 常驻，不返回
      return;
    }
