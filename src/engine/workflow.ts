...
import { buildAcceptanceContract, initializeAcceptanceEvidence } from '../model/acceptance.js';
export { findTemplateExpressions, renderValue } from '../shared/template.js';
import { findTemplateExpressions, renderValue } from '../shared/template.js';
...
function normalizeStep(step, index, context) {
  ...
  if (type === 'command') {
    const out = { ...base, command: step.command };
    if (step.forEach !== undefined) {
      // forEach 步：source 渲染（steps.*.rows 延迟占位），params 含循环变量 {{<as>.x}}，
      // 运行时逐项渲染，故 plan 阶段保留原样不渲染，避免误报 UNRESOLVED。
      out.forEach = renderValue(step.forEach, context);
      out.as = step.as || 'item';
      out.params = step.params || {};
    } else {
      out.params = redactSensitive(renderValue(step.params || {}, context));
    }
    if (step.when !== undefined) out.when = renderValue(step.when, context);
    return out;
  }
  return {
    ...base,
...
// 组合工作流：任一 step 通过 `command` 引用其它命令 → 可被 workflow 执行引擎真正执行
// （区别于只含 request/ui 的 recipe，那类仍是 plan-only）。
export function hasComposableWorkflow(command) {
  const steps = normalizeRecipe(command)?.steps;
  return Array.isArray(steps) && steps.some((step) => typeof step?.command === 'string' && step.command);
}
