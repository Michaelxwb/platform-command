// @ts-nocheck
import { commandResourceRoot, loadCommand } from '../model/command_store.js';
import { resolveCommandParams } from '../model/params_resolver.js';
import { buildWorkflowPlan, normalizeRecipe, renderValue, hasComposableWorkflow } from './workflow.js';
import { buildAcceptanceContract, initializeAcceptanceEvidence, evaluateAcceptance } from '../model/acceptance.js';
import { describeSessionRef } from '../adapter/session.js';
...
import { hasStoreOp, executeStoreCommand } from './store_command.js';
import { recordRun } from '../io/runs.js';
...
export function getExecutionCapability(command) {
  // 组合工作流（steps 含 command 引用）由 workflow 执行引擎真正执行——先于其它判定，
  // 因为这类命令本身无 dataSource/单步 UI，旧逻辑会误判为 plan-only。
  if (hasComposableWorkflow(command)) {
    return {
      executable: true,
      engine: 'workflow_compose',
      mode: 'compose',
      reason: 'Composed workflow command orchestrates sub-commands via the workflow executor.'
    };
  }
  // store CRUD 命令由 store 引擎执行（本地状态，无需登录态）。
  if (hasStoreOp(command)) {
    return {
      executable: true,
      engine: 'store_op',
      mode: 'store',
      reason: 'Store command reads/writes the platform store layer via the store engine.'
    };
  }
  // 浏览器响应拦截流（导出链路）：需 server-mode storageState 会话；否则仅 dry-run 计划。
  if (command.interceptFlow) {
    if (resolveServerMode().storageStatePath) {
      return {
        executable: true,
        engine: 'ui_intercept',
        mode: 'intercept',
        reason: 'Intercept-flow command runs via Playwright route rewrite + capture + poll under the storageState session.'
      };
    }
    return {
      executable: false,
      engine: 'ui_intercept',
      mode: 'intercept_plan',
      reason: 'Intercept-flow command requires a browser session. 本地先用 `platform-command session import-cookie --host <域名> --cookie "<整串>"` 导入登录态，再设 PLATFORM_COMMAND_USER_ID 与 PLATFORM_COMMAND_STORAGE_STATE；dry-run planning is supported.'
    };
  }
...
export async function executeCommand(commandName, providedParams = {}, options = {}) {
  ...
  if (capability.executable) {
    ...
    try {
      ...
      const { closePlaywright } = await import('../adapter/playwright_adapter.js');
...
async function checkWebbridgeReadiness(command, adapters, blockers) {
  try {
    const { checkWebbridge } = await import('../adapter/webbridge.js');
...
async function checkPlaywrightReadiness(command, adapters, blockers, serverMode) {
  ...
  if (!targetUrl) return;
  const { getSessionState } = await import('../adapter/session_state.js');
  const { STORAGE_STATE_IMPORT_GUIDE } = await import('../adapter/playwright_adapter.js');
